...
class DPLLDE:
    ...
    -- Ablation: Mutation Rule (Contributions 2 & 3) ONLY --
    ...
    Commented-out contributions:
      # 1. Sequential Level Assignment — disabled; k_t fixed to 1 for all levels.
    ...
    # [C1] RESET_INTERVAL not needed without sequential level assignment.
    ...
    def __init__(
        ...
        self.S_count            = 0
        # [C1] k_o and k_t not driven by sequential assignment; k_t fixed to 1
        # self.k_o                = np.zeros(NL, dtype=int)
        self.k_t                = np.ones(NL, dtype=int)   # fixed: elite level
    ...
    # =========================================================================
    # [C1] DISABLED in this ablation — function removed.
    ...
    # =========================================================================
    # Diversity-Aware Difference Vector Selection  (DP-LLDE contribution 2) — ACTIVE
    # =========================================================================
    ...
    # =========================================================================
    # DP-LLDE Mutation  (DP-LLDE contribution 3) — ACTIVE
    # =========================================================================
    ...
    def mutate(
        ...
        Note: with [C1] disabled, level_k is always 1, so scaling = (NL-1)/NL
    ...
    def optimize(self, verbose: bool = True) -> Tuple[np.ndarray, float, List[float], List[float]]:
        ...
        # [C1] sequential level state not used; k_t stays fixed at 1
        ...
        while FES < self.max_fes:
            ...
            # [C1] Sequential level assignment disabled — k_t remains 1 for all levels
            ...
            for level_idx in range(self.NL):
                level_k = self.k_t[level_idx]      # always 1 in this ablation
                ...
                for j in range(self.LS):
                    ...
                    # DP-LLDE mutation ([C2]+[C3] active; [C1] disabled)
                    mutant = self.mutate(
                        population, fitness,
                        target, global_idx,
...
if __name__ == "__main__":
    print("=" * 70)
    print("DP-LLDE (Mutation Rule Only) — Quick Smoke Test")
    print("=" * 70)
