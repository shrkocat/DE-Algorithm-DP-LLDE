...
class DPLLDE:
    ...
    -- Ablation: Level Selection (Contribution 1) ONLY --
    ...
    Commented-out contributions:
      # 2. Diversity-Aware Exemplar Selection — disabled; r1/r2 picked uniformly.
      # 3. DP-LLDE Mutation with Normalized Pressure — disabled; plain (r1 - r2)
      #    used, no (NL-k)/NL scaling factor.
    ...
    def __init__(
        ...
        mu_F: float = 0.5,
        c: float = 0.1,
        # diversity_threshold_ratio: float = 0.1,  # [C2] not used in this ablation
        max_fes: int = 10000,
        ...
        # diversity_threshold_ratio : float  # [C2] disabled
        ...
        self.mu_F           = mu_F
        self.c              = c
        # self.diversity_threshold_ratio = diversity_threshold_ratio  # [C2] disabled
        ...
        # self.D_0                = None            # [C2] not needed without diversity
        # self.diversity_threshold = None           # [C2] not needed without diversity
        self.S_count            = 0
    ...
    # =========================================================================
    # Sequential Level Assignment  (DP-LLDE contribution 1) — ACTIVE
    # =========================================================================
    ...
    # =========================================================================
    # [C2] DISABLED in this ablation — function removed.
    # def _select_diverse_pair(self, pool, exclude_idx=None):
    #     """
    ...
    def mutate(
        ...
        """
        current-to-pbest/1 mutation. [C2] diversity-aware pair selection
        disabled; r1/r2 drawn uniformly at random. [C3] normalized pressure
        scaling disabled; plain (r1 - r2) used instead.

            V_i = X_i + F*(X_pbest - X_i) + F*(r1 - r2)
        """
        ...
        # --- r1, r2 selection (plain uniform random — [C2] diversity check disabled) ---
        if level_idx == 0:
            ...
            available = [j for j in range(len(pool)) if j != local_pos]
            if len(available) < 2:
                idx1, idx2 = 0, min(1, len(pool) - 1)
            else:
                idx1, idx2 = np.random.choice(available, size=2, replace=False)
            r1, r2 = pool[idx1].copy(), pool[idx2].copy()
            # [C2] r1, r2 = self._select_diverse_pair(pool, exclude_idx=local_pos)
        else:
            ...
            # [C2] diversity check on r2 relative to r1 — disabled
            ...
            if len(r2_candidates) == 0:
                ...
                # if np.linalg.norm(r1 - r2) < self.diversity_threshold:  # [C2] disabled
                #     dists = np.linalg.norm(r2_candidates - r1, axis=1)
                #     r2 = r2_candidates[int(np.argmax(dists))].copy()

        # [C3] normalised pressure scaling — disabled
        # scaling = (self.NL - level_k) / self.NL   # ∈ [0, (NL-1)/NL]

        mutant = target + F * (X_pbest - target) + F * (r1 - r2)
        # [C3] mutant = target + F * (X_pbest - target) + F * (r1 - r2) * scaling
        return mutant
    ...
    def optimize(self, verbose: bool = True) -> Tuple[np.ndarray, float, List[float], List[float]]:
        ...
        # [C2] diversity threshold setup not needed without contribution 2
        ...
        diversity_history    = [self.compute_diversity(population)]  # Track diversity per generation
        ...
        while FES < self.max_fes:
            ...
            for level_idx in range(self.NL):
                ...
                for j in range(self.LS):
                    ...
                    # Mutation (contribution 1 active; [C2]+[C3] disabled)
                    mutant = self.mutate(
                        population, fitness,
                        target, global_idx,
...
if __name__ == "__main__":
    print("=" * 70)
    print("DP-LLDE (Level Selection Only) — Quick Smoke Test")
    print("=" * 70)
